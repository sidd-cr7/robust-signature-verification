...
def predict_image(image_path):
    ...
    if adv_pred.item() == 0:
        return {'status': 'adversarial', 'message': 'Adversarial Attack Detected'}
    ...
    if forg_pred.item() == 0:
        return {'status': 'forged', 'message': 'Forged Signature'}
    else:
        return {'status': 'genuine', 'message': 'Genuine Signature'}
